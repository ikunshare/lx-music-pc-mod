...
const api_ikun = {
  getMusicUrl(songInfo, type) {
    const requestObj = httpFetch(
      `https://lxmusic.ikunshare.com/url?source=kg&songId=${songInfo._types[type].hash}&quality=${type}`,
      {
        method: 'get',
        timeout,
        headers,
    ...
    requestObj.promise = requestObj.promise.then(({ statusCode, body }) => {
      ...
      switch (body.code) {
        ...
        case 1:
          return Promise.reject(new Error(requestMsg.ipblock))
        case 2:
          return Promise.reject(new Error(requestMsg.fail))
        case 5:
          return Promise.reject(new Error(requestMsg.ohh))
      }
    })
    return requestObj
  },
